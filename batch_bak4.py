...
# Essential protection to keep your Master/Dummies untouched
...
for i, filename in enumerate(frame_files):
    ...
    # Identify ONLY the actual data meshes (excluding Nodes)
    meshes = [o for o in new_objs if o.type == 'MESH' and "node" not in o.name.lower()]
    meshes.sort(key=lambda o: [int(c) if c.isdigit() else c.lower() for c in re.split('(\\\\d+)', o.name)])
    
    current_frame = i + 1 

    for slot_idx, obj in enumerate(meshes):
        # A) Connect to Controller (Parenting keeps them organized)
        obj.parent = cntrl
        
        # B) Master Material Linking
        if slot_idx < len(m_names):
            target_mat = bpy.data.materials.get(m_names[slot_idx])
            if target_mat:
                obj.data.materials.clear()
                obj.data.materials.append(target_mat)
                obj.color = target_mat.diffuse_color

        # C) Keyframe Animation (Visibility)
        is_dummy_slot = len(obj.data.vertices) <= 1
        obj.scale = (0, 0, 0)
        obj.keyframe_insert(data_path="scale", frame=current_frame - 1)
        obj.scale = (1, 1, 1) if not is_dummy_slot else (0, 0, 0)
...
# 3. GLOBAL HIDE (The safe alternative to deleting)
# Hide every object that is not a protected template item or a child of our controller
for o in bpy.data.objects:
    is_protected = any(p in o.name for p in protected)
    if not is_protected and o.parent != cntrl:
        o.hide_viewport = True
        o.hide_render = True
        # Move them to a separate collection to clean up the outliner view
        if "IMPORT_JUNK" not in bpy.data.collections:
            junk_coll = bpy.data.collections.new("IMPORT_JUNK")
            bpy.context.scene.collection.children.link(junk_coll)
        else:
            junk_coll = bpy.data.collections["IMPORT_JUNK"]
            
        for coll in o.users_collection:
            coll.objects.unlink(o)
        junk_coll.objects.link(o)

bpy.context.scene.frame_end = len(frame_files)
bpy.context.scene.frame_set(1)
print("Sync finished. Junk moved to 'IMPORT_JUNK' collection.")
"""
...
def generate_blender_script_one(path):
    ...
    blender_script = f"""# created with BatchMol {ver_no} (Ultra-Safe Sort)
import bpy, re, os
