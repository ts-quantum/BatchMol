...
# Important: Anything containing "MASTER" or "DUMMY" must be ignored for the timeline!
...
# 2. Strict Discovery: ONLY newly imported meshes
# We filter out anything that is protected or a Master/Dummy
all_objs = [o for o in bpy.data.objects if o.type == 'MESH' and not any(p in o.name for p in protected)]

# Robust Numeric Sorting (Natural Sort)
def get_numeric_suffix(obj):
    matches = frame_pattern.findall(obj.name)
    return int(matches[-1]) if matches else 0

all_objs.sort(key=get_numeric_suffix)

print(f"Debug: Processing {{len(all_objs)}} imported objects into {{len(all_objs)//slots_per_frame}} frames.")

# 3. Processing and Grouping
for i, obj in enumerate(all_objs):
    # Calculate Slot and Frame index
    frame_idx = (i // slots_per_frame) + 1
    slot_idx = i % slots_per_frame # 0=Mol, 1=Pos, 2=Neg, 3=Surf
    
    # Parenting only for frame objects
    obj.parent = cntrl
    
    # A) Master Material Assignment by SLOT
    target_mat = None
    if slot_idx == 0: target_mat = "MASTER_Molecule"
    elif slot_idx == 1: target_mat = "MASTER_Orb_Pos"
    elif slot_idx == 2: target_mat = "MASTER_Orb_Neg"
    elif slot_idx == 3: target_mat = "MASTER_Surface"
    
    if target_mat:
        mat = bpy.data.materials.get(target_mat)
        if mat:
            obj.data.materials.clear()
            obj.data.materials.append(mat)

    # B) Animation (Visibility Toggle)
    # Check for dummies from PyVista (often 1 vertex, 0 faces)
    is_dummy_geom = len(obj.data.polygons) == 0
    
    obj.scale = (0, 0, 0)
    obj.keyframe_insert(data_path="scale", frame=frame_idx - 1)
    
    if not is_dummy_geom:
        obj.scale = (1, 1, 1)
    else:
        obj.scale = (0, 0, 0)
...
# 4. Final Scene Setup
total_frames = len(all_objs) // slots_per_frame
bpy.context.scene.frame_start = 1
bpy.context.scene.frame_end = total_frames if total_frames > 0 else 1
bpy.context.scene.frame_set(1)

print("Sync complete. Dummies protected. Frames animated.")
"""
    with open(script_path, "w") as f:
        f.write(blender_script)
